				   ...
				   int img_dbl, int descr_width, int descr_hist_bins )
{
	...
	write_imgData("txt_opencv_gauss[0][0].txt", gauss_pyr[0][0]->imageData, 307200*4);
	write_imgData("txt_opencv_gauss[0][1].txt", gauss_pyr[0][1]->imageData, 307200 * 4);
	write_imgData("txt_opencv_gauss[0][2].txt", gauss_pyr[0][2]->imageData, 307200 * 4);
	write_imgData("txt_opencv_gauss[0][3].txt", gauss_pyr[0][3]->imageData, 307200 * 4);


	cv::Mat opencv_gauss;
	opencv_gauss = cvarrToMat(gauss_pyr[0][0]);
	cv::Mat imgshow_gauss(opencv_gauss.rows, opencv_gauss.cols, CV_8UC1);
	opencv_gauss.convertTo(imgshow_gauss, CV_8UC1);
	cv::imshow("opencv_gauss", opencv_gauss);
	cv::imshow("imgshow_gauss", imgshow_gauss);
	//cv::Mat opencv_dog(dog_pyr[0][0]);
	cv::imwrite("IMG_opencv_gauss.bmp", opencv_gauss);
	cv::imwrite("IMG_opencv_imgshow_gauss.bmp", imgshow_gauss);
	//cv::imwrite("IMG_opencv_dog.bmp", opencv_dog);
...
static IplImage* create_init_img( IplImage* img, int img_dbl, double sigma )
{
	...
	write_imgData("txt_oepncv_img.txt", img->imageData, 307200);
	write_imgData("txt_oepncv_gray32.txt", gray->imageData, 307200 * 4);
...
static IplImage*** build_dog_pyr( IplImage*** gauss_pyr, int octvs, int intvls )
{
	...
	for( o = 0; o < octvs; o++ )
		...
		//cvShowImage("dog_pyr[0][0]", dog_pyr[0][0]);
		//cvShowImage("dog_pyr[0][1]", dog_pyr[0][3]);
		//cvShowImage("dog_pyr[0][2]", dog_pyr[0][4]);
...
{
	CvSeq* features;
	double prelim_contr_thr = 0.5 * contr_thr / intvls;// 0.5 *0.04 /3 = 0.00666
	struct feature* feat;
	struct detection_data* ddata;
	int o, i, r, c;

	printf("contr_thr = %f ,intvls = %d ,prelim_contr_thr = %f \r\n", contr_thr, intvls, prelim_contr_thr);
	features = cvCreateSeq( 0, sizeof(CvSeq), sizeof(struct feature), storage );
	for( o = 0; o < octvs; o++ )
		for( i = 1; i <= intvls; i++ )
			for(r = SIFT_IMG_BORDER; r < dog_pyr[o][0]->height-SIFT_IMG_BORDER; r++)
				for(c = SIFT_IMG_BORDER; c < dog_pyr[o][0]->width-SIFT_IMG_BORDER; c++)//忽略边缘5个像素
					/* perform preliminary check on contrast */
					if( ABS( pixval32f( dog_pyr[o][i], r, c ) ) > prelim_contr_thr )
						if( is_extremum( dog_pyr, o, i, r, c ) )// 求极值
						{
							feat = interp_extremum(dog_pyr, o, i, r, c, intvls, contr_thr);
							if( feat )
							{
								ddata = feat_detection_data( feat );
								if( ! is_too_edge_like( dog_pyr[ddata->octv][ddata->intvl],//删除边缘效应
										...
										int r, int c, int intvls, double contr_thr )
{
	...
	//printf("contr_thr = %f, contr_thr = %d \r\n", contr_thr, intvls);
						 ...
						 double* xi, double* xr, double* xc )
{
	...
	dD = deriv_3D( dog_pyr, octv, intvl, r, c );////计算三个方向的梯度（雅克比矩阵）
	H = hessian_3D(dog_pyr, octv, intvl, r, c); // 计算3维空间的hessian矩阵

	H_inv = cvCreateMat( 3, 3, CV_64FC1 );
	cvInvert( H, H_inv, CV_SVD );//计算还塞矩阵的逆矩阵
